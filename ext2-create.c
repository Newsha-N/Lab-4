...
void write_block_group_descriptor_table(int fd) {
	...
	block_group_descriptor.bg_free_blocks_count = NUM_FREE_BLOCKS; // not sure about this one agian
	block_group_descriptor.bg_free_inodes_count = NUM_FREE_INODES; // same as the above
	block_group_descriptor.bg_used_dirs_count = 2; // also not sure about this look again
...
void write_block_bitmap(int fd) {
	...
	u8 bitmap[BLOCK_SIZE]={0}; //1024 blocks 
	for (int i= 0 ; i < 3 ; i++)
	 {
		 bitmap[i] = 0xFF;//255 bits 
		 // 8 one bits and to access every
	 }
	for (int i= 3 ; i < 128 ; i++)
	{
		bitmap[i] = 0x0;
	}
	bitmap[2] = 0x7f; // bitmap 2 you fill 7 bits of it 
	for (int i=128 ; i <1024 ; i++)
	{
		bitmap[i]=0xFF;
		// not having access to the remaining ones 
	}
	bitmap[127] = 0x80;
...
void write_inode_bitmap(int fd) {
	...
	u8 bitmap[BLOCK_SIZE]={0}; //why?
	bitmap[0] = 0xff;
	bitmap[1] = 0x1f;
	for (int i= 2 ; i < 16 ; i++)
	 {
		 bitmap[i] = 0x0;//255 bits 
		 // 8 one bits and to access every
	 }
	for (int i= 16 ; i < 1024 ; i++)
	{
	 ...
	 // bitmap 2 you fill 7 bits of it 
...
struct ext2_inode root_dir_inode = {0};
	...
	memcpy(hello_inode.i_block, "hello-world", 11); // copy 11 bytes to the inode block 
	// it copies the content of the file rather than the name  
...
void write_root_dir_block(int fd) {
	...
	// what is the parent directory of the root
	...
	struct ext2_dir_entry lost_entry = {0};
	dir_entry_set(lost_entry, LOST_AND_FOUND_INO, "lost+found");
	dir_entry_write(lost_entry, fd);
	bytes_remaining -= lost_entry.rec_len;
...
void write_hello_world_file_block(int fd) {
	...
	ssize_t size = 12;
	if (write(fd, "Hello world", size) != size) {
		errno_exit("write");
	}
}
